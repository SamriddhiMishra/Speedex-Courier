...
def career(request):
    ...
    if request.method=='POST' and request.FILES['myfile']:
        ...
        if Career.objects.filter(emailaddress=emailaddress).exists():
            ...
            fs.save(cv,myfile)
            msg='Your application is submitted!!'
...
def savecomplain(request):
    ...
    en=Complain(name=name,gender=gender,address=address,contactno=contactno,emailaddress=emailaddress,subject=subject,comlplaintext=complaintext,complaindate=complaindate)
    en.save()
    return redirect('index')
