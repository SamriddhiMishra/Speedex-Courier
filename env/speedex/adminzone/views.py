...
def addnotification(request):
    if request.session['userid']:
        ...
        postddate=datetime.datetime.now().strftime('%Y/%m/%d')
        n=Notification(notificationtext=notificationtext,postddate=postddate)
        n.save()
...
def deletejobseekers(request,emailaddress):
    if request.session['userid']:
        j=Career.objects.get(emailaddress=emailaddress)
        j.delete()
